...
namespace StandaloneTestScene
{
    public class Level
    {
        ...
        private bool CollisionDetection(Vector3 position, float x, float z)
        {
			...
			foreach (var entity in Entities.OfType<GoalParticles>())
            {
            ...
            {
				Assets.Sounds.Effects.PickupKey.Play();
        ...
        public void Load(Texture2D level)
        {
            ...
            for (int x = 0; x < level.Width; x++)
            {
                for (int y = 0; y < level.Height; y++)
                {
                    ...
                    {
						Entities.Add(new GoalParticles(new Vector3(x*Settings.TILE_WIDTH, 0, y*Settings.TILE_HEIGHT), 60, 1000, 0.01f));
                    }
                }
            }
